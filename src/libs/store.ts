...
const demoData: Clothes[] = Array.from({length: 21}, (_, index) => ({
    id: index + 1,
    clothes: `Shirt${index ? index : ''}`,
    color: "Blue",
    rate: 100,
    qty: 2,
...
export const useStore = create<StateStore>((set) => ({
    ...
    searchData: (search: string) => set(() => ({
        clothes: search ? demoData.filter((item) => item.clothes.toLowerCase().includes(search.trim().toLowerCase())) : demoData
    })),
    addOrder: (data) => set((state) => ({
        orders: state.orders.find((item) => item.id === data.id)
            ? state.orders.map((item) => item.id === data.id ? {...item, qty: item.qty + 1} : item)
            : [...state.orders, data]
    })),
    updateQty: (id, qty) => set((state) => ({
        orders: state.orders.map((item) => item.id === id ? {...item, qty} : item)
    })),
    deleteOrder: (id) => set((state) => ({
        orders: state.orders.filter((item) => item.id !== id)
    })),
    addNotes: (notes) => set({notes}),
    sendPayment: () => set((state) => {
        console.log(state.orders, state.notes, state.paid);
        return initialState;
    }),
